...
| Logging speed and clarity | `P1` | Default logging path must be sub-60-second on mobile, show one planned workout surface, avoid duplicate workout cards, prefill planned details, keep notes/review/history collapsed by default, and make the common save path obvious. | `PASS` | The current Log surface now behaves like an execution tool: one planned-session reference, large run/lift controls, quiet save state, and collapsed audit/history by default. The known `duplicated log content` issue remains closed by regression coverage. | [`e2e/log-prescribed-workflow.spec.js`](../e2e/log-prescribed-workflow.spec.js), [`tests/workout-log-form-service.test.js`](../tests/workout-log-form-service.test.js), [`docs/LOG_SURFACE_EXECUTION.md`](./LOG_SURFACE_EXECUTION.md) |
| Copy quality and consumer tone | `P1` | Default screens should be highlights-first, internal language must not appear in normal user flows, secondary rationale should be collapsed instead of filling the top layer, and the main redesigned surfaces should stay inside a first-load clarity budget. | `AT RISK` | Internal-language and first-load clarity guards are now automated for Today, Program, Log, and Coach, including visible-text budgets, single-primary-CTA checks, duplicate-current-day prevention, and collapsed-detail checks. Manual premium-read review is still needed before launch. | [`e2e/consumer-copy-guard.spec.js`](../e2e/consumer-copy-guard.spec.js), [`e2e/surface-clarity-guard.spec.js`](../e2e/surface-clarity-guard.spec.js), [`docs/CONTENT_REDUCTION_INVENTORY.md`](./CONTENT_REDUCTION_INVENTORY.md), [`docs/INTERNAL_LANGUAGE_REMOVAL_AUDIT.md`](./INTERNAL_LANGUAGE_REMOVAL_AUDIT.md), [`docs/DESIGN_AUDIT_EXECUTION_REPORT.md`](./DESIGN_AUDIT_EXECUTION_REPORT.md), [`docs/PRODUCT_DELETION_REPORT.md`](./PRODUCT_DELETION_REPORT.md) |
...
| --- | --- | --- | --- | --- | --- |
| `LR-101` | Screen density still needs a final manual premium-read pass after the bootstrap blocker is fixed. | Copy quality, mobile usability | `OPEN` | Automated first-load clarity guards now exist, but a human premium-read pass is still pending for laptop density, typography rhythm, and light-mode feel. | Yes, if the final manual pass is completed before public launch. |
| `LR-102` | Overlong screens are reduced, but not yet protected by automated per-screen text budgets. | Copy quality | `CLOSED` | [`e2e/surface-clarity-guard.spec.js`](../e2e/surface-clarity-guard.spec.js) now enforces per-surface first-load visible-word budgets plus CTA and collapsed-detail guardrails for Today, Program, Log, and Coach. | Closed. |
...
- `overlong screens`
  - Current status: `AT RISK`
  - Evidence: [`docs/CONTENT_REDUCTION_INVENTORY.md`](./CONTENT_REDUCTION_INVENTORY.md), [`docs/PRODUCT_DELETION_REPORT.md`](./PRODUCT_DELETION_REPORT.md), [`e2e/surface-clarity-guard.spec.js`](../e2e/surface-clarity-guard.spec.js)
...
| Surface clarity guard | `cmd /c npx playwright test e2e/surface-clarity-guard.spec.js` | Today, Program, Log, and Coach stay inside first-load CTA, duplicate-card, collapsed-detail, and visible-word budgets. | `PASS` |
...
- Additional scroll-depth regression coverage beyond the current first-load text-budget guard.
