...
Launch is now blocked by one remaining P0 theme:
...
| Adaptive rollout safety | `P2` | Adaptive must stay deterministic by default or remain shadow-only until shadow coverage is large enough, harmful cohorts are zero, and at least one decision point is eligible for a limited rollout. | `AT RISK` | The new rollout gate ran on the fixture pack and correctly blocked activation: `144` shadow rows, `1` harmful cohort, `0` eligible decision points, recommendation `keep_in_shadow`. The adversarial adaptive-readiness audit also says live adaptive prescription is not ready until exposure logging, stable join keys, and a durable event sink exist. | [`npm run qa:adaptive-policy:launch-readiness`](../package.json), [`npm run qa:adaptive-policy:promote`](../package.json), [`artifacts/adaptive-launch-readiness/results.json`](../artifacts/adaptive-launch-readiness/results.json), [`docs/ADAPTIVE_POLICY_PROMOTION_WORKFLOW.md`](./ADAPTIVE_POLICY_PROMOTION_WORKFLOW.md), [`docs/ADAPTIVE_LEARNING_LAUNCH_SAFETY.md`](./ADAPTIVE_LEARNING_LAUNCH_SAFETY.md), [`docs/ADAPTIVE_LEARNING_ADVERSARIAL_AUDIT.md`](./ADAPTIVE_LEARNING_ADVERSARIAL_AUDIT.md), [`docs/ADAPTIVE_LEARNING_ENABLEMENT_CHECKLIST.md`](./ADAPTIVE_LEARNING_ENABLEMENT_CHECKLIST.md), [`docs/ADAPTIVE_LEARNING_BACKLOG.md`](./ADAPTIVE_LEARNING_BACKLOG.md) |
...
| Security and account recovery | `P0` | Password reset must work from the auth gate, through the in-app recovery link, and from signed-in Settings. Auth flows must not accidentally dump signed-in users into onboarding or local-only fallback when account actions are triggered. | `PASS` | `e2e/password-reset.spec.js` passed `3/3`, and the signed-in logout/account fallbacks also passed after the signed-in boot race fix. | [`e2e/password-reset.spec.js`](../e2e/password-reset.spec.js), [`e2e/auth-and-management.spec.js`](../e2e/auth-and-management.spec.js), [`e2e/account-lifecycle.spec.js`](../e2e/account-lifecycle.spec.js), [`docs/AUTH_AND_SUPABASE_RELIABILITY_SPEC.md`](./AUTH_AND_SUPABASE_RELIABILITY_SPEC.md) |
...
| `LR-203` | Adaptive rollout is structurally ready, but no decision point is currently eligible for activation on the fixture shadow gate and harmful cohorts are still flagged. | Adaptive rollout safety | `OPEN` | `npm run qa:adaptive-policy:launch-readiness:fixture` produced `144` shadow rows, `1` harmful cohort, and `0` eligible decision points in [`artifacts/adaptive-launch-readiness/results.json`](../artifacts/adaptive-launch-readiness/results.json). | Yes, as long as adaptive remains off or shadow-only. |
