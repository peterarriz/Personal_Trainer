...
export const createPersistedPayloadFingerprint = (payload = {}) => createStableFingerprint(
  stripFingerprintNoise(payload || {})
);

const DEFAULT_PERSIST_QUEUE_INVALIDATION_RESULT = Object.freeze({
  ok: false,
  skipped: true,
  stale: true,
...
const clonePersistQueueInvalidationResult = (result = DEFAULT_PERSIST_QUEUE_INVALIDATION_RESULT) => ({
  ...DEFAULT_PERSIST_QUEUE_INVALIDATION_RESULT,
...
export const createPersistQueueController = ({ execute } = {}) => {
  ...
  const invalidateQueued = (result = DEFAULT_PERSIST_QUEUE_INVALIDATION_RESULT) => {
    if (!state.queuedEntry) return false;
    const queuedEntry = state.queuedEntry;
    state.queuedEntry = null;
    queuedEntry.resolve(clonePersistQueueInvalidationResult(result));
    return true;
  };
